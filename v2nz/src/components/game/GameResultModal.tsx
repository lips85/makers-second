...
import { Trophy, Target, Zap, Clock, RotateCcw, Home, TrendingUp, Award, Star } from 'lucide-react'
...
export function GameResultModal({ 
  ...
  const performanceRating = Math.round((stats.accuracy * 0.4 + (stats.maxCombo / 10) * 100 * 0.3 + (totalTime / stats.averageResponseTime) * 100 * 0.3))

  // 스테나인 색상 매핑
  const getStanineColor = (stanine: number) => {
    if (stanine >= 8) return 'text-purple-600 bg-purple-100 dark:bg-purple-900/20'
    ...
    if (stanine >= 2) return 'text-yellow-600 bg-yellow-100 dark:bg-yellow-900/20'
        ...
        <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="text-center p-4 bg-gradient-to-br from-green-50 to-emerald-100 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg">
                <div className="flex items-center justify-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-gray-600 dark:text-gray-400">퍼센타일</span>
                </div>
                <div className="text-3xl font-bold text-green-600 mb-1">
                  {percentileStats.percentile.toFixed(1)}%
                </div>
                <div className={`text-sm font-medium ${getPercentileGrade(percentileStats.percentile).color}`}>
              ...
              <div className="text-center p-4 bg-gradient-to-br from-orange-50 to-red-100 dark:from-orange-900/20 dark:to-red-900/20 rounded-lg">
                <div className="flex items-center justify-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-gray-600 dark:text-gray-400">스테나인</span>
                </div>
                <div className="text-3xl font-bold text-orange-600 mb-1">
                  {percentileStats.stanine}
                </div>
                <Badge className={`${getStanineColor(percentileStats.stanine)} border-0`}>
                  {percentileStats.stanine >= 8 ? '우수' : 
                   percentileStats.stanine >= 6 ? '양호' : 
                   percentileStats.stanine >= 4 ? '보통' : 
                   percentileStats.stanine >= 2 ? '미흡' : '부족'}
                </Badge>
              </div>
            </div>
          )}
          ...
          <div className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div className="flex items-center justify-center gap-2 mb-3">
              ...
              <span className="text-lg font-semibold text-gray-900 dark:text-white">정확도</span>
            </div>
            <div className="relative inline-flex items-center justify-center">
              <svg className="w-24 h-24 transform -rotate-90">
                ...
                <circle
                  ...
                  strokeDashoffset={`${2 * Math.PI * 36 * (1 - stats.accuracy / 100)}`}
          ...
          <div className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-lg">
            <div className="flex items-center justify-center gap-2 mb-3">
              ...
              <span className="text-lg font-semibold text-gray-900 dark:text-white">평균 응답 속도</span>
            </div>
            <div className="relative">
              <Progress 
                value={Math.min(100, (1000 / Math.max(stats.averageResponseTime / 1000, 0.5)) * 100)} 
                className="h-3 mb-2"
              />
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {stats.averageResponseTime < 2000 ? '매우 빠름' : 
                 stats.averageResponseTime < 4000 ? '빠름' : 
                 stats.averageResponseTime < 6000 ? '보통' : '느림'}
              </p>
            </div>
          </div>
          ...
          {leaderboard && (
            <div className="text-center p-4 bg-gradient-to-br from-yellow-50 to-amber-100 dark:from-yellow-900/20 dark:to-amber-900/20 rounded-lg">
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <span className="text-lg font-semibold text-gray-900 dark:text-white">리더보드 순위</span>
              </div>
          ...
          <div className="space-y-2 text-sm bg-gray-50 dark:bg-gray-800 p-4 rounded-lg">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{((stats.correctAnswers / stats.totalQuestions) * 100).toFixed(1)}%</span>
            </div>
            {stats.maxCombo > 1 && (
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">콤보 횟수:</span>
