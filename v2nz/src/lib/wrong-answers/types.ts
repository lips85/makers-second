...
// 로컬 스토리지 키 규약
export const WRONG_ANSWERS_STORAGE_KEY = 'v2nz_wrong_answers'
...
export function sortWrongAnswerItems(
  ...
  return [...items].sort((a, b) => {
    ...
    switch (sortBy) {
      case 'lastWrongAt':
        comparison = new Date(a.lastWrongAt).getTime() - new Date(b.lastWrongAt).getTime()
        break
      case 'wrongCount':
 ...
 * 오답 아이템 생성
 */
export function createWrongAnswerItem(
  wordItem: {
    id: string
    word: string
    meaning: string
 ...
 * 오답 아이템 마스터 표시
 */
export function markWrongAnswerAsMastered(item: WrongAnswerItem): WrongAnswerItem {
  return {
    ...item,
 ...
 * 오답 아이템 틀린 횟수 증가
 */
export function incrementWrongAnswerCount(item: WrongAnswerItem): WrongAnswerItem {
  return {
    ...item,
